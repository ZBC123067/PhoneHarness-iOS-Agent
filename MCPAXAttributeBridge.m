...
typedef struct {
    ...
    int (*axValueGetType)(CFTypeRef value);
    ...
    AXUIElementIsAttributeSettableFunc isAttributeSettable;
...
static CFStringRef MCPAXBridgeSemanticProbeAXAttributeForName(NSString *name) {
    if ([name isEqualToString:@"AXRole"]) return kAXRoleAttribute;
    if ([name isEqualToString:@"AXSubrole"]) return kAXSubroleAttribute;
    if ([name isEqualToString:@"AXEnabled"]) return kAXEnabledAttribute;
    if ([name isEqualToString:@"AXFocused"]) return kAXFocusedAttribute;
    if ([name isEqualToString:@"AXValue"]) return kAXValueAttribute;
    return NULL;
}

static NSString *MCPAXBridgeSemanticProbeErrorCategory(AXError error) {
    if (error == kAXErrorAttributeUnsupported) return @"attribute_unsupported";
    if (error == kAXErrorNoValue) return @"no_value";
    if (error == kAXErrorCannotComplete) return @"cannot_complete";
    if (error == kAXErrorInvalidUIElement) return @"invalid_element";
    if (error == kAXErrorNotImplemented) return @"not_implemented";
    return @"bridge_failure";
}

static NSString *MCPAXBridgeSemanticBatchObjectCategory(id value) {
    if (!value || value == NSNull.null) return @"null_object";

    CFTypeID typeID = 0;
    @try {
        typeID = CFGetTypeID((__bridge CFTypeRef)value);
    } @catch (__unused NSException *exception) {
        return @"unknown_wrapper";
    }

    if (typeID == CFBooleanGetTypeID()) return @"boolean";
    if (typeID == CFNumberGetTypeID()) return @"number";
    if (typeID == CFStringGetTypeID()) {
        NSString *string = [(NSString *)value lowercaseString];
        // Only sentinel-shaped strings are AX error placeholders, matching the
        // node-source sentinel filter. A real label that merely mentions
        // "axerror" is a value and must survive as one.
        if ([string containsString:@"error:-252"] || [string containsString:@"kaxvalueaxerrortype"]) {
            return @"error_object";
        }
        return @"string";
    }

    if (sMCPAXBridgeRuntime.axValueGetTypeID &&
        sMCPAXBridgeRuntime.axValueGetType &&
        typeID == sMCPAXBridgeRuntime.axValueGetTypeID()) {
        int axValueType = 0;
        @try {
            axValueType = sMCPAXBridgeRuntime.axValueGetType((__bridge CFTypeRef)value);
        } @catch (__unused NSException *exception) {
            return @"unknown_wrapper";
        }
        return axValueType == 5 ? @"error_object" : @"ax_value";
    }

    NSString *className = [NSStringFromClass([value class]) lowercaseString];
    if ([className containsString:@"error"]) return @"error_object";
    return @"unknown_wrapper";
}

static BOOL MCPAXBridgeIsExplicitlyUnavailableAttributeValue(id value) {
    NSString *category = MCPAXBridgeSemanticBatchObjectCategory(value);
    return [category isEqualToString:@"null_object"] ||
           [category isEqualToString:@"error_object"];
}

static BOOL MCPAXBridgeSemanticProbeValueIsValid(id value, NSString *attribute) {
    NSString *category = MCPAXBridgeSemanticBatchObjectCategory(value);
    if ([attribute isEqualToString:@"AXRole"] || [attribute isEqualToString:@"AXSubrole"]) {
        return [category isEqualToString:@"string"] &&
            [(NSString *)value stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet].length > 0;
    }
    if (![category isEqualToString:@"boolean"] && ![category isEqualToString:@"number"]) return NO;
    double number = [(NSNumber *)value doubleValue];
    if ([attribute isEqualToString:@"automationType"]) {
        return [category isEqualToString:@"number"] && number >= 0 && number <= 1000 &&
            number == (double)[(NSNumber *)value integerValue];
    }
    if ([attribute isEqualToString:@"AXEnabled"] || [attribute isEqualToString:@"AXFocused"] ||
        [attribute isEqualToString:@"isVisible"] || [attribute isEqualToString:@"isUserInteractionEnabled"]) {
        return number == 0.0 || number == 1.0;
    }
    return NO;
}

static NSString *MCPAXBridgeSemanticProbeAttributeToken(NSString *name) {
    static NSDictionary<NSString *, NSString *> *tokens;
...
static BOOL MCPAXBridgeSemanticProbeNamesContainAttribute(NSSet<NSString *> *names,
                                                          CFStringRef attribute) {
    if (!names || !attribute) return NO;
    return [names containsObject:(__bridge NSString *)attribute];
}
...
static BOOL MCPAXBridgeLoadRuntime(NSString **error) {
    ...
    dispatch_once(&onceToken, ^{
        BOOL (^bindSymbolsFromHandle)(void *, NSString *) = ^BOOL(void *handle, NSString *label) {
            ...
            CFTypeID (*axValueGetTypeID)(void) =
                (CFTypeID (*)(void))dlsym(handle, "AXValueGetTypeID");
            int (*axValueGetType)(CFTypeRef) =
                (int (*)(CFTypeRef))dlsym(handle, "AXValueGetType");
            AXUIElementCreateApplicationFunc createApplication =
            ...
            AXUIElementIsAttributeSettableFunc isAttributeSettable =
                (AXUIElementIsAttributeSettableFunc)dlsym(handle, "AXUIElementIsAttributeSettable");
            AXUIElementCopyElementAtPositionFunc copyElementAtPosition =
            ...
            sMCPAXBridgeRuntime.axValueGetTypeID = axValueGetTypeID;
            sMCPAXBridgeRuntime.axValueGetType = axValueGetType;
            ...
            sMCPAXBridgeRuntime.isAttributeSettable = isAttributeSettable;
...
- (id _Nullable)copyAttributeObject:(AXUIElementRef)element
    ...
    return MCPAXBridgeIsExplicitlyUnavailableAttributeValue(object) ? nil : object;
}

- (BOOL)getAttributeSettable:(BOOL *)settableOut
                  forElement:(AXUIElementRef)element
                   attribute:(CFStringRef)attribute {
    if (settableOut) *settableOut = NO;
    if (!element || !attribute || !sMCPAXBridgeRuntime.isAttributeSettable) return NO;
    MCPAXBridgeSetRequestingClientForAutomation();
    Boolean settable = false;
    AXError error = sMCPAXBridgeRuntime.isAttributeSettable(element, attribute, &settable);
    if (error != kAXErrorSuccess) return NO;
    if (settableOut) *settableOut = settable ? YES : NO;
    return YES;
}

- (NSDictionary<NSString *, NSDictionary<NSString *, id> *> *)semanticAttributeAvailabilityProbeForElement:(AXUIElementRef)element {
    static NSArray<NSDictionary<NSString *, id> *> *definitions;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        definitions = @[
            @{ @"field": @"semantic_role", @"ax": @[@"AXRole", @"AXSubrole"], @"xc": @[@"automationType"], @"settable": @NO },
            @{ @"field": @"editable", @"ax": @[@"AXValue"], @"xc": @[], @"settable": @YES },
            // Role/type reads are counted under semantic_role, not as a secure property.
            @{ @"field": @"secure", @"ax": @[], @"xc": @[], @"settable": @NO },
            @{ @"field": @"enabled", @"ax": @[@"AXEnabled"], @"xc": @[@"isUserInteractionEnabled"], @"settable": @NO },
            @{ @"field": @"focused", @"ax": @[@"AXFocused"], @"xc": @[], @"settable": @NO },
            @{ @"field": @"visible", @"ax": @[], @"xc": @[@"isVisible"], @"settable": @NO },
            @{ @"field": @"actionable", @"ax": @[@"AXValue"], @"xc": @[@"isUserInteractionEnabled"], @"settable": @YES }
        ];
    });
    ...
    if (!element) {
        for (NSDictionary *definition in definitions) {
            result[definition[@"field"]] = @{
                ...
                @"ax_batch_read_success": @NO,
                @"ax_single_read_success": @NO,
                @"xc_read_success": @NO,
                @"settable_or_availability": @NO,
                @"unavailable": @YES,
                @"error_categories": @[@"invalid_element"]
            };
        }
        return [result copy];
    }
    ...
    @try {
        ...
        BOOL nameEnumerationSupported = sMCPAXBridgeRuntime.copyAttributeNames != NULL;
        BOOL nameEnumerationSucceeded = NO;
        NSString *nameEnumerationCategory = nil;
        NSSet<NSString *> *attributeNames = nil;
        if (nameEnumerationSupported) {
            CFArrayRef names = NULL;
            AXError error = sMCPAXBridgeRuntime.copyAttributeNames(element, &names);
            if (error == kAXErrorSuccess && names) {
                NSArray *array = CFBridgingRelease(names);
                if ([array isKindOfClass:[NSArray class]]) {
                    attributeNames = [NSSet setWithArray:array];
                    nameEnumerationSucceeded = YES;
                } else {
                    nameEnumerationCategory = @"attribute_name_enumeration_invalid";
                }
            } else {
                if (names) CFRelease(names);
                nameEnumerationCategory = [NSString stringWithFormat:@"attribute_name_enumeration_%@",
                                           MCPAXBridgeSemanticProbeErrorCategory(error)];
            }
        } else {
            nameEnumerationCategory = @"attribute_name_enumeration_unsupported";
        }

        NSArray<NSString *> *safeAXNames = @[@"AXRole", @"AXSubrole", @"AXEnabled", @"AXFocused"];
        NSMutableDictionary<NSString *, NSNumber *> *batchRead = [NSMutableDictionary dictionary];
        NSMutableDictionary<NSString *, NSString *> *batchObjectCategories = [NSMutableDictionary dictionary];
        NSMutableDictionary<NSString *, NSString *> *batchIndexNames = [NSMutableDictionary dictionary];
        NSString *batchCategory = nil;
        if (sMCPAXBridgeRuntime.copyMultipleAttributeValues) {
            NSMutableArray *attributes = [NSMutableArray arrayWithCapacity:safeAXNames.count];
            for (NSString *name in safeAXNames) {
                CFStringRef attribute = MCPAXBridgeSemanticProbeAXAttributeForName(name);
                if (attribute) [attributes addObject:(__bridge id)attribute];
            }
            ...
            if (error == kAXErrorSuccess && values) {
                NSArray *array = CFBridgingRelease(values);
                if ([array isKindOfClass:[NSArray class]]) {
                    NSArray<NSString *> *indexWords = @[@"zero", @"one", @"two", @"three"];
                    for (NSUInteger index = 0; index < MIN(array.count, safeAXNames.count); index++) {
                        id value = array[index];
                        NSString *name = safeAXNames[index];
                        NSString *category = MCPAXBridgeSemanticBatchObjectCategory(value);
                        batchObjectCategories[name] = category;
                        batchIndexNames[name] = indexWords[index];
                        batchRead[name] = @(MCPAXBridgeSemanticProbeValueIsValid(value, name));
                    }
                } else {
                    batchCategory = @"batch_invalid";
                }
            } else {
                if (values) CFRelease(values);
                batchCategory = [NSString stringWithFormat:@"batch_%@",
                                 MCPAXBridgeSemanticProbeErrorCategory(error)];
            }
        } else {
            batchCategory = @"batch_unsupported";
        }

        NSMutableDictionary<NSString *, NSNumber *> *singleRead = [NSMutableDictionary dictionary];
        NSString *singleCategory = nil;
        if (sMCPAXBridgeRuntime.copyAttributeValue) {
            for (NSString *name in safeAXNames) {
                CFStringRef attribute = MCPAXBridgeSemanticProbeAXAttributeForName(name);
                if (!attribute) continue;
                CFTypeRef value = NULL;
                AXError error = sMCPAXBridgeRuntime.copyAttributeValue(element, attribute, &value);
                singleRead[name] = @(error == kAXErrorSuccess && value != NULL &&
                                    MCPAXBridgeSemanticProbeValueIsValid((__bridge id)value, name));
                if (value) CFRelease(value);
                if (error != kAXErrorSuccess && !singleCategory) {
                    singleCategory = [NSString stringWithFormat:@"single_%@",
                                      MCPAXBridgeSemanticProbeErrorCategory(error)];
                }
            }
        } else {
            singleCategory = @"single_unsupported";
        }

        NSArray<NSString *> *safeXCKeys = @[@"automationType", @"isUserInteractionEnabled", @"isVisible"];
        NSMutableDictionary<NSString *, NSNumber *> *xcRead = [NSMutableDictionary dictionary];
        NSString *xcCategory = nil;
        if (sMCPAXBridgeRuntime.copyAttributeValue) {
            for (NSString *key in safeXCKeys) {
                CFStringRef attribute = MCPAXBridgeXCAttributeRefForKey(key);
                if (!attribute) {
                    xcRead[key] = @NO;
                    if (!xcCategory) xcCategory = @"xc_unsupported";
                    continue;
                }
                CFTypeRef value = NULL;
                AXError error = sMCPAXBridgeRuntime.copyAttributeValue(element, attribute, &value);
                xcRead[key] = @(error == kAXErrorSuccess && value != NULL &&
                               MCPAXBridgeSemanticProbeValueIsValid((__bridge id)value, key));
                if (value) CFRelease(value);
                if (error != kAXErrorSuccess && !xcCategory) {
                    xcCategory = [NSString stringWithFormat:@"xc_%@",
                                  MCPAXBridgeSemanticProbeErrorCategory(error)];
                }
            }
        } else {
            xcCategory = @"xc_unsupported";
        }

        BOOL valueSettable = NO;
        BOOL settableObserved = [self getAttributeSettable:&valueSettable
                                                forElement:element
                                                 attribute:kAXValueAttribute];

        for (NSDictionary *definition in definitions) {
            NSString *field = definition[@"field"];
            NSArray<NSString *> *axNames = definition[@"ax"];
            NSArray<NSString *> *xcKeys = definition[@"xc"];
            BOOL probesSettable = [definition[@"settable"] boolValue];
            BOOL hasSensitiveAXValue = [axNames containsObject:@"AXValue"];
            BOOL attributePresent = NO;
            BOOL batchSuccess = NO;
            BOOL singleSuccess = NO;
            BOOL xcSuccess = NO;
            NSMutableArray<NSString *> *categories = [NSMutableArray array];
            if ([field isEqualToString:@"secure"]) [categories addObject:@"direct_secure_property_unavailable"];

            for (NSString *name in axNames) {
                CFStringRef attribute = MCPAXBridgeSemanticProbeAXAttributeForName(name);
                if (nameEnumerationSucceeded && MCPAXBridgeSemanticProbeNamesContainAttribute(attributeNames, attribute)) {
                    attributePresent = YES;
                }
                if (!hasSensitiveAXValue) {
                    if ([batchRead[name] boolValue]) batchSuccess = YES;
                    if ([singleRead[name] boolValue]) singleSuccess = YES;
                }
                NSString *objectCategory = batchObjectCategories[name];
                NSString *indexName = batchIndexNames[name];
                NSString *attributeToken = MCPAXBridgeSemanticProbeAttributeToken(name);
                if (objectCategory.length > 0 && indexName.length > 0 && attributeToken.length > 0) {
                    [categories addObject:[NSString stringWithFormat:@"batch_%@_index_%@_%@",
                                           attributeToken,
                                           indexName,
                                           objectCategory]];
                }
            }
            for (NSString *key in xcKeys) {
                if ([xcRead[key] boolValue]) xcSuccess = YES;
            }

            if (!nameEnumerationSucceeded && axNames.count > 0) {
                [categories addObject:nameEnumerationCategory ?: @"attribute_name_enumeration_failed"];
            } else if (axNames.count > 0 && !attributePresent) {
                [categories addObject:@"attribute_absent"];
            }
            if (hasSensitiveAXValue) {
                [categories addObject:@"content_read_skipped"];
            } else if (attributePresent && !batchSuccess) {
                [categories addObject:batchCategory ?: @"attribute_present_but_batch_read_failed"];
            }
            if (!hasSensitiveAXValue && attributePresent && !singleSuccess) {
                [categories addObject:singleCategory ?: @"attribute_present_but_single_read_failed"];
            }
            if (xcKeys.count > 0 && !xcSuccess) {
                [categories addObject:xcCategory ?: @"xc_state_unavailable"];
            }
            if (probesSettable && !settableObserved) {
                [categories addObject:@"availability_probe_unavailable"];
            }

            BOOL availability = probesSettable && settableObserved;
            BOOL unavailable = !(batchSuccess || singleSuccess || xcSuccess || availability);
            result[field] = @{
                @"attribute_present": @(attributePresent),
                @"ax_batch_read_success": @(batchSuccess),
                @"ax_single_read_success": @(singleSuccess),
                @"xc_read_success": @(xcSuccess),
                @"settable_or_availability": @(availability),
        ...
        for (NSDictionary *definition in definitions) {
            result[definition[@"field"]] = @{
                ...
                @"settable_or_availability": @NO,
                @"unavailable": @YES,
                @"error_categories": @[@"bridge_exception"]
            };
        }
    }
    return [result copy];
...
- (NSDictionary<NSString *, id> * _Nullable)copyAttributeMap:(AXUIElementRef)element
    ...
    for (NSInteger idx = 0; idx < count; idx++) {
        ...
        if (MCPAXBridgeIsExplicitlyUnavailableAttributeValue(value)) continue;
