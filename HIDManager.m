...
// Batch 1D E1-B experiment: single keyboard keycode tap (down + up) through
// the existing postHIDEvent broadcast path (same sender value as the touch
// events). Experiment build only — not part of the production input contract.
- (void)expKeyboardTapWithPage:(uint32_t)page usage:(uint32_t)usage completion:(void (^)(BOOL, NSString *))completion {
    dispatch_async(_hidQueue, ^{
        NSString *error = nil;
        if (![self sendKeyboardEventWithPage:page usage:usage down:YES error:&error]) {
            if (completion) completion(NO, error);
            return;
        }
        usleep(30000);
        if (![self sendKeyboardEventWithPage:page usage:usage down:NO error:&error]) {
            if (completion) completion(NO, error);
            return;
        }
        if (completion) completion(YES, nil);
