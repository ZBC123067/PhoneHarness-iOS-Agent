...
// Bounds the per-leaf availability probe (each probe costs ~10 synchronous
// main-thread AX calls). The semantic-state path is unaffected; diagnostics
// beyond the limit are reported as skipped instead of probing.
static const NSUInteger kMCPAXNodeSemanticProbeLimit = 300;
...
// Semantic state must not coerce strings, error wrappers, or arbitrary numbers.
static NSNumber *MCPAXNodeSemanticBoolean(id value) {
    if (![value isKindOfClass:[NSNumber class]]) return nil;
    double number = [(NSNumber *)value doubleValue];
    if (number != 0.0 && number != 1.0) return nil;
    return @(number == 1.0);
}
...
static NSString *MCPAXNodeCompactElementFingerprint(NSDictionary *element) {
    ...
    NSString *fieldIdentifier = [element[@"identifier"] isKindOfClass:[NSString class]] ?
        element[@"identifier"] : nil;
    if (fieldIdentifier.length > 0) {
        // Exact direct AX identity wins over the legacy render fingerprint.
        // This prevents an unidentified proxy encountered first from erasing
        // the identified leaf, without correlating by text or geometry.
        return [@"field-identifier:" stringByAppendingString:fieldIdentifier];
    }
#endif
...
static NSDictionary *MCPAXNodeSemanticState(id value, NSString *source) {
    id normalized = value ?: @"unknown";
    NSString *normalizedSource = source.length > 0 ? source : @"unavailable";
    return @{ @"value": normalized, @"source": normalizedSource };
}

static NSArray<NSString *> *MCPAXNodeSemanticDiagnosticFieldNames(void) {
    return @[
        @"semantic_role", @"editable", @"secure", @"enabled",
        @"focused", @"visible", @"actionable"
    ];
}

static NSArray<NSString *> *MCPAXNodeSemanticDiagnosticCounterNames(void) {
    return @[
        @"attribute_present_count",
        @"ax_batch_read_success_count",
        @"ax_single_read_success_count",
        @"xc_read_success_count",
        @"settable_or_availability_count",
        @"unavailable_count"
    ];
}

static NSMutableDictionary *MCPAXNodeCreateSemanticDiagnostics(void) {
    NSMutableDictionary *fields = [NSMutableDictionary dictionary];
    for (NSString *field in MCPAXNodeSemanticDiagnosticFieldNames()) {
        NSMutableDictionary *counts = [NSMutableDictionary dictionary];
        for (NSString *counter in MCPAXNodeSemanticDiagnosticCounterNames()) {
            counts[counter] = @0;
        }
        counts[@"error_category_counts"] = [NSMutableDictionary dictionary];
        fields[field] = counts;
    }
    return [@{
        @"schema": @"phoneharness.test52.ax_source_probe.v1",
        @"node_count": @0,
        @"same_leaf_identity_count": @0,
        @"fields": fields
    } mutableCopy];
}

static void MCPAXNodeAccumulateSemanticDiagnostics(NSMutableDictionary *diagnostics,
                                                   NSDictionary<NSString *, NSDictionary<NSString *, id> *> *probe) {
    if (![diagnostics isKindOfClass:[NSMutableDictionary class]] ||
        ![probe isKindOfClass:[NSDictionary class]]) {
        return;
    }
    diagnostics[@"node_count"] = @([diagnostics[@"node_count"] integerValue] + 1);
    // Reusing an AX reference does not establish one immutable device snapshot.
    NSMutableDictionary *fields = [diagnostics[@"fields"] isKindOfClass:[NSMutableDictionary class]] ?
        diagnostics[@"fields"] : nil;
    if (!fields) return;

    NSDictionary<NSString *, NSString *> *counterKeys = @{
        @"attribute_present": @"attribute_present_count",
        @"ax_batch_read_success": @"ax_batch_read_success_count",
        @"ax_single_read_success": @"ax_single_read_success_count",
        @"xc_read_success": @"xc_read_success_count",
        @"settable_or_availability": @"settable_or_availability_count",
        @"unavailable": @"unavailable_count"
    };
    for (NSString *field in MCPAXNodeSemanticDiagnosticFieldNames()) {
        NSDictionary *fieldProbe = [probe[field] isKindOfClass:[NSDictionary class]] ? probe[field] : nil;
        NSMutableDictionary *counts = [fields[field] isKindOfClass:[NSMutableDictionary class]] ? fields[field] : nil;
        if (!fieldProbe || !counts) continue;
        [counterKeys enumerateKeysAndObjectsUsingBlock:^(NSString *probeKey, NSString *counterKey, BOOL *stop) {
            (void)stop;
            if ([fieldProbe[probeKey] boolValue]) {
                counts[counterKey] = @([counts[counterKey] integerValue] + 1);
            }
        }];
        NSArray *categories = [fieldProbe[@"error_categories"] isKindOfClass:[NSArray class]] ?
            fieldProbe[@"error_categories"] : @[];
        NSMutableDictionary *categoryCounts = [counts[@"error_category_counts"] isKindOfClass:[NSMutableDictionary class]] ?
            counts[@"error_category_counts"] : nil;
        for (id category in categories) {
            if (![category isKindOfClass:[NSString class]] || !categoryCounts) continue;
            NSString *categoryName = (NSString *)category;
            if (categoryName.length == 0) continue;
            categoryCounts[categoryName] = @([categoryCounts[categoryName] integerValue] + 1);
        }
    }
}

static NSString *MCPAXNodeSHA256(NSString *value) {
...
static NSString *MCPAXNodeSemanticRole(NSNumber *automationType,
                                       NSString *axRole,
                                       NSString *axSubrole,
                                       NSString **sourceOut) {
    NSInteger type = automationType.integerValue;
    if (automationType) {
        if (type == 45) { if (sourceOut) *sourceOut = @"xc_attribute"; return @"search_field"; }
        if (type == 49) { if (sourceOut) *sourceOut = @"xc_attribute"; return @"text_field"; }
        if (type == 50) { if (sourceOut) *sourceOut = @"xc_attribute"; return @"secure_text_field"; }
        if (type == 52) { if (sourceOut) *sourceOut = @"xc_attribute"; return @"editable_text_field"; }
        if (type == 9) { if (sourceOut) *sourceOut = @"xc_attribute"; return @"button"; }
        if (type == 42) { if (sourceOut) *sourceOut = @"xc_attribute"; return @"link"; }
        if (type == 31) { if (sourceOut) *sourceOut = @"xc_attribute"; return @"browser"; }
    }

    NSString *combined = [[NSString stringWithFormat:@"%@ %@", axRole ?: @"", axSubrole ?: @""] lowercaseString];
    if (combined.length > 1) {
        if ([combined containsString:@"searchfield"] || [combined containsString:@"search field"]) {
            if (sourceOut) *sourceOut = @"ax_attribute";
            return @"search_field";
        }
        if ([combined containsString:@"secure"] && [combined containsString:@"text"]) {
            if (sourceOut) *sourceOut = @"ax_attribute";
            return @"secure_text_field";
        }
        if ([combined containsString:@"textfield"] || [combined containsString:@"text field"]) {
            if (sourceOut) *sourceOut = @"ax_attribute";
            return @"text_field";
        }
        if ([combined containsString:@"textarea"] || [combined containsString:@"textview"]) {
            if (sourceOut) *sourceOut = @"ax_attribute";
            return @"editable_text_field";
        }
        if ([combined containsString:@"button"]) {
            if (sourceOut) *sourceOut = @"ax_attribute";
            return @"button";
        }
        if ([combined containsString:@"link"]) {
            if (sourceOut) *sourceOut = @"ax_attribute";
            return @"link";
        }
    }
...
- (NSDictionary * _Nullable)serializeCompactLeafElement:(AXUIElementRef)leafElement
    ...
    if (includeSemanticState) {
        NSMutableArray *safeAttributes = [numericAttributes mutableCopy];
        [safeAttributes removeObject:@(kMCPAXNodeAttributeValue)];
        numericAttributes = safeAttributes;
    }
    ...
    NSMutableArray<NSString *> *xcKeys = [@[
        @"automationType",
        @"frame",
        @"isUserInteractionEnabled",
        @"isVisible",
...
#ifdef PH_TEST52_FIELD_EVIDENCE
    [xcKeys addObject:@"identifier"];
#endif
    if (includeSemanticState) [xcKeys removeObject:@"value"];
    NSDictionary<NSString *, id> *xcValues = [self.attributeBridge copyXCAttributeMap:leafElement attributeKeys:xcKeys] ?: @{};
    NSMutableArray<NSString *> *axAttributeNames = includeSemanticState ? [@[
            @"AXRole",
            @"AXSubrole",
            @"AXEnabled",
            @"AXFocused",
...
#ifdef PH_TEST52_FIELD_EVIDENCE
    [axAttributeNames addObject:(__bridge NSString *)kAXIdentifierAttribute];
#endif
    NSDictionary<NSString *, id> *axValues = axAttributeNames.count > 0 ?
        ([self.attributeBridge copyAttributeMap:leafElement attributes:axAttributeNames] ?: @{}) : @{};
    NSDictionary<NSString *, id> *directValues = [self.attributeBridge copyDirectAttributeMap:leafElement attributeKeys:@[
        @"centerPoint",
        @"focusableFrameForZoom",
        @"userInputLabels",
    ...
    NSNumber *isVisible = includeSemanticState ? MCPAXNodeSemanticBoolean(xcValues[@"isVisible"]) :
        MCPAXNodeNumberFromValue(xcValues[@"isVisible"]);
    ...
    MCPAXNodeAddCompactText(texts, xcValues[@"placeholderValue"]);
    MCPAXNodeAddCompactText(texts, axValues[@"AXPlaceholderValue"]);
    if (!includeSemanticState) {
...
#ifdef PH_TEST52_FIELD_EVIDENCE
    id xcIdentifierValue = xcValues[@"identifier"];
    id axIdentifierValue = axValues[(__bridge NSString *)kAXIdentifierAttribute];
    NSString *fieldIdentifier = MCPAXNodeStringFromValue(xcIdentifierValue) ?:
        MCPAXNodeStringFromValue(axIdentifierValue);
    id fieldSnapshot = nil;
    id snapshotIdentifierValue = nil;
    if (fieldIdentifier.length == 0) {
        // Some iOS 17 leaf proxies omit identifier from ordinary AX/XC
        // attribute reads but expose it in that same leaf's parameterized
        // user-testing snapshot.  Request only identifier so no label, value,
        // geometry, or hierarchy data can become a correlation substitute.
        fieldSnapshot = [self.attributeBridge copyXCParameterizedAttributeObject:leafElement
            attributeKey:@"userTestingSnapshotParameterized"
            parameter:@{@"attributes": @[@"identifier"], @"UseLegacyElementType": @NO,
                        @"preserveRemoteElementPlaceholders": @NO}];
        if ([fieldSnapshot isKindOfClass:[NSDictionary class]]) {
            snapshotIdentifierValue = MCPAXNodeSnapshotAttributeValue(fieldSnapshot,
                kMCPAXNodeSnapshotAttributeIdentifier) ?: fieldSnapshot[@"identifier"];
            fieldIdentifier = MCPAXNodeStringFromValue(snapshotIdentifierValue);
        }
    }
    if (fieldIdentifier.length > 0) element[@"identifier"] = fieldIdentifier;
    element[@"_phfe_identifier_probe"] = @{
        @"xc_present": @(xcIdentifierValue != nil),
        @"xc_string": @(MCPAXNodeStringFromValue(xcIdentifierValue).length > 0),
        @"ax_present": @(axIdentifierValue != nil),
        @"ax_string": @(MCPAXNodeStringFromValue(axIdentifierValue).length > 0),
        @"snapshot_present": @(fieldSnapshot != nil),
        @"snapshot_identifier_present": @(snapshotIdentifierValue != nil),
        @"snapshot_string": @(MCPAXNodeStringFromValue(snapshotIdentifierValue).length > 0)
    };
#endif
    ...
    if (includeSemanticState) {
        NSNumber *automationType = [xcValues[@"automationType"] isKindOfClass:[NSNumber class]] ? xcValues[@"automationType"] : nil;
        if (automationType && (!(automationType.doubleValue >= 0 && automationType.doubleValue <= 1000) ||
            automationType.doubleValue != (double)automationType.integerValue)) automationType = nil;
        NSString *axRole = [axValues[@"AXRole"] isKindOfClass:[NSString class]] ? axValues[@"AXRole"] : nil;
        NSString *axSubrole = [axValues[@"AXSubrole"] isKindOfClass:[NSString class]] ? axValues[@"AXSubrole"] : nil;
        NSString *roleSource = nil;
        NSString *semanticRole = MCPAXNodeSemanticRole(automationType, axRole, axSubrole, &roleSource);

        BOOL valueSettable = NO;
        BOOL settableObserved = [self.attributeBridge getAttributeSettable:&valueSettable
                                                               forElement:leafElement
                                                                attribute:kAXValueAttribute];
        NSDictionary *editableState = settableObserved ?
            MCPAXNodeSemanticState(@(valueSettable), @"ax_attribute") :
            MCPAXNodeSemanticState(@"unknown", @"unavailable");

        BOOL secureKnown = NO;
        BOOL secure = NO;
        NSString *secureSource = @"unavailable";
        if (automationType && automationType.integerValue == 50) {
            secureKnown = YES; secure = YES; secureSource = @"inferred";
        } else if (automationType &&
                   (automationType.integerValue == 45 || automationType.integerValue == 49 || automationType.integerValue == 52)) {
            secureKnown = YES; secure = NO; secureSource = @"inferred";
        } else {
            NSString *roleText = [[NSString stringWithFormat:@"%@ %@", axRole ?: @"", axSubrole ?: @""] lowercaseString];
            if ([roleText containsString:@"secure"] && [roleText containsString:@"text"]) {
                secureKnown = YES; secure = YES; secureSource = @"inferred";
            }
        }

        NSNumber *enabledValue = MCPAXNodeSemanticBoolean(axValues[@"AXEnabled"]);
        NSString *enabledSource = enabledValue ? @"ax_attribute" : @"unavailable";
        if (!enabledValue) {
            enabledValue = MCPAXNodeSemanticBoolean(xcValues[@"isUserInteractionEnabled"]);
            if (enabledValue) enabledSource = @"xc_attribute";
        }
        NSNumber *focusedValue = MCPAXNodeSemanticBoolean(axValues[@"AXFocused"]);
        NSNumber *interactionValue = MCPAXNodeSemanticBoolean(xcValues[@"isUserInteractionEnabled"]);

        NSMutableDictionary *semanticState = [@{
            @"semantic_role": MCPAXNodeSemanticState(semanticRole, roleSource),
            @"editable": editableState,
            @"secure": secureKnown ? MCPAXNodeSemanticState(@(secure), secureSource) : MCPAXNodeSemanticState(@"unknown", @"unavailable"),
            @"enabled": enabledValue ? MCPAXNodeSemanticState(@(enabledValue.boolValue), enabledSource) : MCPAXNodeSemanticState(@"unknown", @"unavailable"),
            @"focused": focusedValue ? MCPAXNodeSemanticState(@(focusedValue.boolValue), @"ax_attribute") : MCPAXNodeSemanticState(@"unknown", @"unavailable"),
            @"visible": isVisible ? MCPAXNodeSemanticState(@(isVisible.boolValue), @"xc_attribute") : MCPAXNodeSemanticState(@(onScreen), @"inferred"),
            @"actionable": interactionValue ? MCPAXNodeSemanticState(@(interactionValue.boolValue), @"xc_attribute") :
                (settableObserved ? MCPAXNodeSemanticState(@(valueSettable), @"ax_attribute") : MCPAXNodeSemanticState(@"unknown", @"unavailable")),
            @"value_match": MCPAXNodeSemanticState(@"unknown", @"unavailable"),
            @"same_leaf_identity": @"unknown",
            @"same_leaf_identity_source": @"unavailable"
        } mutableCopy];

        // Type-derived nonsecure classification cannot authorize a value probe.
        if (expectedValueSHA256.length == 64 && secureKnown && !secure &&
            ([secureSource isEqualToString:@"ax_attribute"] || [secureSource isEqualToString:@"xc_attribute"])) {
            id rawValue = xcValues[@"value"] ?: numericValues[@(kMCPAXNodeAttributeValue)];
            NSString *rawString = MCPAXNodeStringFromValue(rawValue);
            if (rawString) {
                NSString *actualDigest = MCPAXNodeSHA256(rawString);
                NSString *valueSource = xcValues[@"value"] ? @"xc_attribute" : @"ax_attribute";
                semanticState[@"value_match"] = MCPAXNodeSemanticState(@([actualDigest isEqualToString:expectedValueSHA256]), valueSource);
            }
        }
        element[@"semantic_state"] = semanticState;
    }
    (void)pid;
    (void)bundleId;
...
- (NSDictionary * _Nullable)compactElementsForPid:(pid_t)pid
    ...
    __block NSString *resultError = nil;
    CFAbsoluteTime semanticStart = CFAbsoluteTimeGetCurrent();
    ...
    [self.attributeBridge performOnMainThreadSync:^{
        @try {
            ...
            NSMutableDictionary *semanticDiagnostics = includeSemanticDiagnostics ?
                MCPAXNodeCreateSemanticDiagnostics() : nil;
            ...
            __block NSUInteger probedLeafCount = 0;
            __block NSUInteger probeSkippedCount = 0;
            ...
            void (^appendCompactElement)(AXUIElementRef, NSString *, NSDictionary *) =
                ...
                if (includeSemanticDiagnostics && semanticDiagnostics) {
                    // Bounded main-thread budget: the availability probe costs
                    // ~10 synchronous AX calls per leaf, so deep probes stop at
                    // the limit and the remainder is reported as skipped.
                    if (probedLeafCount < kMCPAXNodeSemanticProbeLimit) {
                        probedLeafCount++;
                        NSDictionary *probe = [self.attributeBridge semanticAttributeAvailabilityProbeForElement:leafElement];
                        MCPAXNodeAccumulateSemanticDiagnostics(semanticDiagnostics, probe);
                    } else {
                        probeSkippedCount++;
                    }
                }
                ...
                mutable[@"path"] = path;
                if (includeSemanticState) mutable[@"semantic_leaf_ref"] = path;
                (void)source;
                (void)extra;
            ...
            payload[@"semantic_state_requested"] = @(includeSemanticState);
            if (includeSemanticState) {
                payload[@"semantic_state_elapsed_ms"] = @((CFAbsoluteTimeGetCurrent() - semanticStart) * 1000.0);
                payload[@"device_generation"] = NSNull.null;
                payload[@"device_generation_source"] = @"unavailable";
            }
            if (includeSemanticDiagnostics && semanticDiagnostics) {
                payload[@"semantic_diagnostics_requested"] = @YES;
                payload[@"semantic_diagnostics"] = [semanticDiagnostics copy];
                payload[@"semantic_probe_limit"] = @(kMCPAXNodeSemanticProbeLimit);
                payload[@"semantic_probe_skipped_count"] = @(probeSkippedCount);
            }
