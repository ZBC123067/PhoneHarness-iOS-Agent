...
- (NSDictionary *)fastFrontmostApplicationInfo {
    ...
    dispatch_block_t block = ^{
        ...
        if (!runtimeAvailable) {
            result = @{
                @"available": @NO,
                @"source": @"ax_frontboard_fast_unavailable"
            };
            return;
        }

        NSDictionary *(^summaryForObject)(id, NSString *) =
        ^NSDictionary *(id applicationObject, NSString *source) {
            NSMutableDictionary *info =
                [NSMutableDictionary dictionary];

            info[@"available"] = @YES;
            info[@"source"] = source ?: @"ax_frontboard_fast";
            ...
            NSString *bundleId =
                [self invokeObjectSelector:@selector(bundleIdentifier)
                                  onTarget:applicationObject];
            ...
            pid_t pid =
                [self pidFromApplicationObject:applicationObject
                                      bundleId:bundleId
                               resolutionTrace:nil];
        ...
        id focusedApps =
            MCPAXFrontBoardCopyObject(
                sMCPAXFrontBoardRuntime.focusedApps
            );

        id frontObject =
            [self preferredApplicationObjectFromCandidateValue:
                focusedApps];

        NSDictionary *info =
            summaryForObject(
                frontObject,
                @"ax_frontboard_focused_apps_fast"
            );

        NSString *bundleId =
            [info[@"bundleId"] isKindOfClass:[NSString class]]
                ? info[@"bundleId"]
                : nil;
        ...
        info =
            summaryForObject(
                focusedProcess,
                @"ax_frontboard_focused_app_process_fast"
            );
        ...
        info =
            summaryForObject(
                frontObject,
                @"ax_frontboard_focused_app_processes_fast"
            );
        ...
        if (sMCPAXFrontBoardRuntime.focusedAppPID) {
            ...
            pidInfo[@"available"] = @YES;
            pidInfo[@"source"] =
                @"ax_frontboard_focused_app_pid_fast";
            pidInfo[@"pid"] = @(focusedPid);

            if (focusedPid > 0) {
                id applicationObject =
                    [self applicationObjectForPid:focusedPid
                                  resolutionTrace:nil];
                ...
                if (applicationObject) {
                    ...
                    [pidInfo addEntriesFromDictionary:objectInfo];
                    pidInfo[@"pid"] = @(focusedPid);
                } else if (focusedPid == getpid()) {
                    pidInfo[@"bundleId"] =
                        @"com.apple.springboard";
                    pidInfo[@"name"] = @"SpringBoard";
                }
            }
        ...
        result = @{
            @"available": @YES,
            @"source": @"ax_frontboard_fast_no_identity"
        };
    };
    ...
    return result ?: @{
        @"available": @NO,
        @"source": @"ax_frontboard_fast_empty"
    };
}
