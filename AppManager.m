...
// Lightweight resolver for launch verification.
// Avoid the full AX/context/scene resolver in a high-frequency polling loop.
static NSDictionary *MCPLightweightFrontmostApplicationInfo(void) {
    ...
    dispatch_block_t block = ^{
        ...
        if (springBoardClass &&
            [springBoardClass respondsToSelector:sharedApplicationSel]) {

            id springBoard = MCPAppMsgSendObject((id)springBoardClass,
                                                 sharedApplicationSel);

            if (springBoard &&
                [springBoard respondsToSelector:frontmostSel]) {
                frontApp = MCPAppMsgSendObject(springBoard, frontmostSel);
            }
        }

        if (frontApp &&
            [frontApp respondsToSelector:@selector(bundleIdentifier)]) {

            NSString *bundleId =
                [frontApp performSelector:@selector(bundleIdentifier)];

            NSString *name = nil;
            if ([frontApp respondsToSelector:@selector(displayName)]) {
                name = [frontApp performSelector:@selector(displayName)];
            }
...
static MCPAXRemoteContextResolver *MCPFastFrontmostResolver(void) {
    static MCPAXRemoteContextResolver *resolver = nil;
    static dispatch_once_t onceToken;

    dispatch_once(&onceToken, ^{
        resolver = [MCPAXRemoteContextResolver new];
    });

    return resolver;
}

static NSDictionary *MCPAuthoritativeFastFrontmostApplicationInfo(void) {
    MCPAXRemoteContextResolver *resolver = MCPFastFrontmostResolver();
    NSDictionary *info = [resolver fastFrontmostApplicationInfo];

    if ([info isKindOfClass:[NSDictionary class]] &&
        [info[@"available"] boolValue]) {
        return info;
    }

    // Compatibility fallback only when AXFrontBoardFocusedAppPID
    // is unavailable on the current OS.
    return MCPLightweightFrontmostApplicationInfo();
}
...
static BOOL MCPWaitForFrontmostApp(NSString *expectedBundleId,
    ...
    NSUInteger consecutiveMatches = 0;

    while ([deadline timeIntervalSinceNow] > 0) {
        lastInfo = MCPAuthoritativeFastFrontmostApplicationInfo();
...
- (BOOL)launchApp:(NSString *)bundleId error:(NSString **)error {
    ...
    // Fast path: target app is already frontmost.
    NSDictionary *currentFrontmost = MCPAuthoritativeFastFrontmostApplicationInfo();
    NSString *currentBundleId =
        [currentFrontmost[@"bundleId"] isKindOfClass:[NSString class]]
            ? currentFrontmost[@"bundleId"]
            : nil;

    if ([currentBundleId isEqualToString:bundleId]) {
        APP_LOG(@"Launch fast-path: %@ is already frontmost", bundleId);
        return YES;
    }

    // Launch primary path: uiopen.
    // On the target RootHide device this avoids the slow/blocking
    // SBUIController activateApplication: path.
    NSString *uiopenPath = MCPResolvedJailbreakPath(@"/usr/bin/uiopen");
    if ([[NSFileManager defaultManager] isExecutableFileAtPath:uiopenPath]) {
        int uiopenExitCode = -1;
        NSString *uiopenError = nil;

        BOOL uiopenRan = MCPRunProcess(
            uiopenPath,
            @[@"--bundleid", bundleId],
            MCPJailbreakEnvironment(),
            1.0,
        ...
        if (uiopenRan && uiopenExitCode == 0) {
            APP_LOG(@"Launched app via uiopen: %@", bundleId);

            NSDictionary *frontmostInfo = nil;
            if (MCPWaitForFrontmostApp(bundleId, 2.0, &frontmostInfo)) {
                APP_LOG(@"uiopen launch confirmed in foreground: %@", bundleId);
                return YES;
            }

            NSString *frontmostBundleId =
                [frontmostInfo[@"bundleId"] isKindOfClass:[NSString class]]
                    ? frontmostInfo[@"bundleId"]
                    : nil;

            if (error) {
                *error = [NSString stringWithFormat:
                    @"uiopen launched %@ but it did not become frontmost within 2 seconds%@",
                    bundleId,
                    frontmostBundleId.length > 0
                        ? [NSString stringWithFormat:@"; current frontmost=%@", frontmostBundleId]
                        : @""];
            }
        ...
        APP_LOG(@"uiopen launch failed for %@, exit=%d error=%@; falling back",
                bundleId,
                uiopenExitCode,
                uiopenError ?: @"-");
    }

    // Compatibility fallback: original SpringBoard / LaunchServices path.
    __block BOOL ok = NO;
...
#pragma clang diagnostic pop
    ...
    if (error) {
        if (frontmostBundleId.length > 0) {
            ...
            *error = [NSString stringWithFormat:@"Launch request sent for %@, but it did not become frontmost within 2 seconds", bundleId];
        }
    }
