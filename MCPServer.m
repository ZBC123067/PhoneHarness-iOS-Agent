#import "MCPServer.h"
#ifdef PH_TEST52_C1_DIAGNOSTIC
#import "diagnostics/test52-c1/PHC1Receiver.h"
#endif
#ifdef PH_TEST52_FIELD_EVIDENCE
#import "diagnostics/test52-field-evidence/PHFEReceiver.h"
#import "diagnostics/test52-field-evidence/PHFEContract.h"
#import <time.h>
#endif
#if defined(PH_TEST52_NATIVE_CAPSULE_CLIENT) && \
    (defined(PH_TEST52_NATIVE_EVIDENCE_V1_ROUTE) || \
     defined(PH_TEST52_NATIVE_EVIDENCE_V2_ROUTE) || \
     defined(PH_TEST52_NATIVE_EVIDENCE_V3_ROUTE) || defined(PH_TEST52_RUNTIME_STATUS_V3))
#import "diagnostics/test52-native-capsule/PHFECapsuleReceiver.h"
#endif
#ifdef PH_TEST52_RUNTIME_STATUS_V3
#import "diagnostics/test52-native-capsule/PHRuntimeStatusV3.h"
#endif
#ifdef PH_TEST52_RUNTIME_STATUS_V2
#import "diagnostics/test52-native-capsule/PHRuntimeStatusV2.h"
#endif
#ifdef PH_TEST52_RUNTIME_STATUS_V1
#import "diagnostics/test52-native-capsule/PHRuntimeStatusV1.h"
#endif
#import "HIDManager.h"
...
#define MCP_VISUAL_SESSION_MAX_ACTIVE 4
#define MCP_VISUAL_SESSION_TTL_SECONDS 120
#define MCP_VISUAL_SESSION_SCOPE @"visual_observation"
...
#ifdef PH_TEST52_FIELD_EVIDENCE
static uint64_t MCPFieldEvidenceNowNanoseconds(void) {
...
static NSString *MCPSecureRandomHexString(void) {
    uint8_t bytes[16] = {0};
    if (SecRandomCopyBytes(kSecRandomDefault, sizeof(bytes), bytes) != errSecSuccess) {
        return nil;
    }

    NSMutableString *value = [NSMutableString stringWithCapacity:sizeof(bytes) * 2];
...
static BOOL MCPIsWellFormedVisualSessionID(NSString *sessionId) {
    if (![sessionId isKindOfClass:[NSString class]] || ![sessionId hasPrefix:@"visualsession."] || sessionId.length != 46) {
        return NO;
    }
    NSString *token = [sessionId substringFromIndex:@"visualsession.".length];
    NSCharacterSet *nonHex = [[NSCharacterSet characterSetWithCharactersInString:@"0123456789abcdef"] invertedSet];
    return [token rangeOfCharacterFromSet:nonHex].location == NSNotFound;
}

static NSString *sMCPAuthToken = nil;
static NSString *sMCPAuthTokenSource = @"UNAVAILABLE";

static BOOL MCPAuthTokenShapeIsValid(NSString *token) {
...
static BOOL MCPConstantTimeEqualStrings(NSString *presented, NSString *expected) {
    NSData *presentedData = [presented isKindOfClass:[NSString class]] ? [presented dataUsingEncoding:NSUTF8StringEncoding] : nil;
    NSData *expectedData = [expected isKindOfClass:[NSString class]] ? [expected dataUsingEncoding:NSUTF8StringEncoding] : nil;
    NSUInteger presentedLength = presentedData ? presentedData.length : 0;
    NSUInteger expectedLength = expectedData ? expectedData.length : 0;
    NSUInteger length = MAX(presentedLength, expectedLength);
    uint8_t difference = (uint8_t)(presentedLength ^ expectedLength);
    const uint8_t *presentedBytes = presentedData ? presentedData.bytes : NULL;
    const uint8_t *expectedBytes = expectedData ? expectedData.bytes : NULL;
    for (NSUInteger index = 0; index < length; index++) {
        uint8_t left = index < presentedLength ? presentedBytes[index] : 0;
        uint8_t right = index < expectedLength ? expectedBytes[index] : 0;
        difference |= (uint8_t)(left ^ right);
    }
    return difference == 0;
}

static void MCPLogAuthStartupDiagnostics(NSString *configured, NSString *resolved, NSString *source) {
    // The diagnostic is intentionally metadata-only: it must distinguish a
    // preference/source mismatch without writing a credential or its hash.
    BOOL debugPreferenceSeen = [MCPLogger isDebugLoggingEnabled];
    if (!debugPreferenceSeen) {
        return;
    }
    MCP_LOG(@"mcp_auth_startup configured_token_present=%@ configured_token_shape_valid=%@ resolved_token_source=%@ resolved_token_length=%lu debug_preference_seen=%@",
            configured ? @"true" : @"false",
            MCPAuthTokenShapeIsValid(configured) ? @"true" : @"false",
            source ?: @"UNAVAILABLE",
            (unsigned long)([resolved isKindOfClass:[NSString class]] ? resolved.length : 0),
            @"true");
}

static NSString *MCPResolveAuthToken(void) {
    // Resolved once per process. On first start a random token is generated
    // and persisted through CFPreferences so it stays stable across
    // SpringBoard restarts. The value is never logged and never echoed in a
    // response; if it cannot be produced every request fails closed.
    if (sMCPAuthToken) {
        return sMCPAuthToken;
    }
    @synchronized ([MCPServer class]) {
        if (sMCPAuthToken) {
            return sMCPAuthToken;
        }
        NSString *configured = IOSMCPConfiguredAuthTokenValue();
        if (MCPAuthTokenShapeIsValid(configured)) {
            sMCPAuthToken = configured;
            sMCPAuthTokenSource = @"PREFERENCE";
            MCPLogAuthStartupDiagnostics(configured, sMCPAuthToken, sMCPAuthTokenSource);
            return sMCPAuthToken;
        }
        NSString *generated = MCPSecureRandomHexString();
        if (!generated) {
            sMCPAuthTokenSource = @"UNAVAILABLE";
            MCPLogAuthStartupDiagnostics(configured, nil, sMCPAuthTokenSource);
            return nil;
        }
        CFPreferencesSetAppValue((__bridge CFStringRef)IOS_MCP_AUTH_TOKEN_PREFERENCE_KEY,
                                 (__bridge CFPropertyListRef)generated,
        ...
        sMCPAuthToken = generated;
        sMCPAuthTokenSource = @"GENERATED";
        MCPLogAuthStartupDiagnostics(configured, sMCPAuthToken, sMCPAuthTokenSource);
        return sMCPAuthToken;
    }
}
...
- (NSDictionary *)executeCreateVisualSession:(id)reqId args:(NSDictionary *)args;
- (NSDictionary *)executeAnalyzeVisualSession:(id)reqId args:(NSDictionary *)args;
- (void)purgeExpiredVisualSessionsAtTime:(NSTimeInterval)now;
- (NSDictionary *)consumeVisualSessionWithID:(NSString *)sessionId atTime:(NSTimeInterval)now;
...
- (NSDictionary *)executeExpKeycode:(id)reqId args:(NSDictionary *)args;
...
- (NSDictionary *)sanitizeUIElementsPayload:(NSDictionary *)payload
                                      debug:(BOOL)debug
                       includeSemanticState:(BOOL)includeSemanticState
                 includeSemanticDiagnostics:(BOOL)includeSemanticDiagnostics;
- (NSDictionary *)sanitizeUIElement:(NSDictionary *)element debug:(BOOL)debug includeSemanticState:(BOOL)includeSemanticState;
...
@implementation MCPServer {
    ...
    NSMutableDictionary<NSString *, NSDictionary *> *_temporaryVisualSessions;
}
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _temporaryVisualSessions = [NSMutableDictionary dictionary];
    }
    return self;
}
...
- (void)startOnPort:(uint16_t)port {
    ...
    BOOL allowLanAccess = IOSMCPAllowLanAccess();
    struct sockaddr_in addr;
    memset(&addr, 0, sizeof(addr));
    addr.sin_family      = AF_INET;
    addr.sin_port        = htons(port);
    // Fail closed by default: loopback only unless the operator explicitly
    // enables LAN access. Token auth is mandatory either way.
    addr.sin_addr.s_addr = allowLanAccess ? htonl(INADDR_ANY) : htonl(INADDR_LOOPBACK);
    ...
    if (!MCPResolveAuthToken()) {
        MCP_LOG(@"MCP auth token unavailable; every request will be rejected until a token can be generated");
    }
    [self purgeStaleUploadFiles];
    MCP_LOG(@"MCP server started on port %d (%s, token auth required)",
            port, allowLanAccess ? "all interfaces" : "loopback only");
}
...
- (void)handleClient:(int)clientSocket {
    ...
    // Fail-closed auth boundary: every endpoint (including /health) requires
    // the X-MCP-Token header. The token value is never logged and never
    // echoed in an error response.
    NSString *presentedToken = headers[@"x-mcp-token"];
    NSString *expectedToken = MCPResolveAuthToken();
    BOOL presentedTokenPresent = [presentedToken isKindOfClass:[NSString class]];
    BOOL expectedTokenPresent = [expectedToken isKindOfClass:[NSString class]];
    BOOL authMatch = expectedTokenPresent && presentedTokenPresent &&
                     MCPConstantTimeEqualStrings(presentedToken, expectedToken);
    if (!authMatch) {
        MCP_LOG(@"mcp_auth_rejected presented_token_present=%@ presented_token_length=%lu expected_token_present=%@ expected_token_length=%lu auth_match=false",
                presentedTokenPresent ? @"true" : @"false",
                (unsigned long)(presentedTokenPresent ? presentedToken.length : 0),
                expectedTokenPresent ? @"true" : @"false",
                (unsigned long)(expectedTokenPresent ? expectedToken.length : 0));
        [self sendErrorResponse:clientSocket status:401 message:@"Unauthorized" requestLogId:requestLogId];
        free(buffer);
        close(clientSocket);
        return;
    }
    ...
    // Route request
#ifdef PH_TEST52_RUNTIME_STATUS_V3
    if ([method isEqualToString:@"POST"] && [basePath isEqualToString:@"/health"]) {
        if (chunkedBody || contentLength <= 0 || contentLength > 4096) {
            [self sendErrorResponse:clientSocket status:400 message:@"Invalid Runtime Status V3 request body" requestLogId:requestLogId];
        } else {
            while (bodyReceived < contentLength && totalRead < HTTP_BUF_SIZE - 1) {
                ssize_t n=read(clientSocket,buffer+totalRead,
                    MIN(HTTP_BUF_SIZE-1-totalRead,contentLength-bodyReceived));
                if (n<=0) break;
                totalRead+=n; bodyReceived+=n;
            }
            NSDictionary *statusRequest=nil;
            if (bodyReceived==contentLength) {
                NSData *bodyData=[NSData dataWithBytes:buffer+headerEnd length:(NSUInteger)contentLength];
                id parsed=[NSJSONSerialization JSONObjectWithData:bodyData options:0 error:nil];
                statusRequest=[parsed isKindOfClass:NSDictionary.class] ? parsed : nil;
            }
            BOOL statusV3=[statusRequest[@"schema"] isEqual:PHRuntimeStatusV3RequestSchema];
            if (statusV3) {
                BOOL mcpRequest=PHRuntimeStatusV3RequestValid(statusRequest,@"mcp");
                BOOL observerRequest=PHRuntimeStatusV3RequestValid(statusRequest,@"observer");
                if (!(mcpRequest || observerRequest)) {
                    [self sendErrorResponse:clientSocket status:400 message:@"Invalid Runtime Status V3 request" requestLogId:requestLogId];
                } else {
                    NSDictionary *runtimeStatus=mcpRequest ? PHRuntimeStatusV3MCPResponse() :
                        PHFERuntimeStatusV3FetchObserver();
                    NSString *component=mcpRequest ? @"mcp" : @"observer";
                    if (!PHRuntimeStatusV3ResponseValid(runtimeStatus,component))
                        runtimeStatus=PHRuntimeStatusV3UnavailableResponse(component,@"LIVE_STATUS_UNAVAILABLE");
                    [self sendJSONResponse:clientSocket status:200 body:runtimeStatus requestLogId:requestLogId];
                }
            }
#ifdef PH_TEST52_RUNTIME_STATUS_V2
            else if ([statusRequest[@"schema"] isEqual:PHRuntimeStatusV2RequestSchema]) {
                BOOL mcpRequest=PHRuntimeStatusV2RequestValid(statusRequest,@"mcp");
                BOOL observerRequest=PHRuntimeStatusV2RequestValid(statusRequest,@"observer");
                if (!(mcpRequest || observerRequest)) {
                    [self sendErrorResponse:clientSocket status:400 message:@"Invalid Runtime Status V2 request" requestLogId:requestLogId];
                } else {
                    NSDictionary *runtimeStatus=mcpRequest ? PHRuntimeStatusV2MCPResponse() :
                        PHFERuntimeStatusV2FetchObserver();
                    NSString *component=mcpRequest ? @"mcp" : @"observer";
                    if (!PHRuntimeStatusV2ResponseValid(runtimeStatus,component))
                        runtimeStatus=PHRuntimeStatusV2UnavailableResponse(component,@"LIVE_STATUS_UNAVAILABLE");
                    [self sendJSONResponse:clientSocket status:200 body:runtimeStatus requestLogId:requestLogId];
                }
            }
#endif
#ifdef PH_TEST52_RUNTIME_STATUS_V1
            else if ([statusRequest[@"schema"] isEqual:PHRuntimeStatusV1RequestSchema]) {
                BOOL mcpRequest=PHRuntimeStatusV1RequestValid(statusRequest,@"mcp");
                BOOL observerRequest=PHRuntimeStatusV1RequestValid(statusRequest,@"observer");
                if (!(mcpRequest || observerRequest)) {
                    [self sendErrorResponse:clientSocket status:400 message:@"Invalid Runtime Status V1 request" requestLogId:requestLogId];
                } else {
                    NSDictionary *runtimeStatus=mcpRequest ? PHRuntimeStatusV1MCPResponse() :
                        PHFERuntimeStatusV1FetchObserver();
                    NSString *component=mcpRequest ? @"mcp" : @"observer";
                    if (!PHRuntimeStatusV1ResponseValid(runtimeStatus,component))
                        runtimeStatus=PHRuntimeStatusV1UnavailableResponse(component,@"LIVE_STATUS_UNAVAILABLE");
                    [self sendJSONResponse:clientSocket status:200 body:runtimeStatus requestLogId:requestLogId];
                }
            }
#endif
            else {
                [self sendErrorResponse:clientSocket status:400 message:@"Invalid Runtime Status version" requestLogId:requestLogId];
            }
        }
    } else
#endif
...
- (NSUInteger)purgeStaleUploadFiles {
    // Stale uploads are removed at server start and before every upload so a
    // planted file cannot linger forever. Per-entry removal failures are
    // logged (never silently swallowed) but do not block fresh uploads; the
    // directory permission check in the upload path is the hard fail-closed
    // boundary.
    NSFileManager *fm = [NSFileManager defaultManager];
    NSError *error = nil;
    NSArray<NSString *> *names = [fm contentsOfDirectoryAtPath:MCP_UPLOAD_DIR error:&error];
    if (!names) {
        if (error && error.code != NSFileReadNoSuchFileError && error.code != NSFileNoSuchFileError) {
            [MCPLogger log:@"upload_purge_failed stage=list dir=<ios-mcp-uploads> error=%@",
    ...
    for (NSString *name in names) {
        NSString *path = [MCP_UPLOAD_DIR stringByAppendingPathComponent:name];
        NSDictionary *attributes = [fm attributesOfItemAtPath:path error:nil];
        NSDate *modified = attributes[NSFileModificationDate];
        if (![modified isKindOfClass:[NSDate class]] || [modified compare:cutoff] != NSOrderedAscending) {
            continue;
        }
        ...
        if ([fm removeItemAtPath:path error:&removeError]) {
            ...
            failed++;
            [MCPLogger log:@"upload_purge_failed stage=remove file=<%@> error=%@",
             MCPLogSafeFileName(name),
             removeError.localizedDescription ?: @"unknown"];
        }
    }
    if (removed > 0 || failed > 0) {
        [MCPLogger log:@"upload_purge removed=%lu failed=%lu",
...
- (void)handleUploadFileRequestPath:(NSString *)path
    ...
    NSFileManager *fm = [NSFileManager defaultManager];
    (void)[self purgeStaleUploadFiles];
    ...
    // The upload directory is security-sensitive: only the owning user may
    // list or plant files here. Repair a pre-existing loose directory, and
    // refuse the upload when it cannot be restricted (fail closed).
    NSError *permError = nil;
    if (![fm setAttributes:@{NSFilePosixPermissions: @0700}
               ofItemAtPath:MCP_UPLOAD_DIR
                     error:&permError]) {
        [self sendErrorResponse:clientSocket status:500 message:[NSString stringWithFormat:@"Failed to restrict upload directory permissions: %@", permError.localizedDescription ?: @"unknown"] requestLogId:requestLogId];
        return;
    }
    NSDictionary *dirAttributes = [fm attributesOfItemAtPath:MCP_UPLOAD_DIR error:nil];
    NSNumber *directoryPermissions = dirAttributes[NSFilePosixPermissions];
    if (dirAttributes == nil ||
        ![directoryPermissions isKindOfClass:[NSNumber class]] ||
        ([directoryPermissions unsignedShortValue] & 077) != 0) {
        [self sendErrorResponse:clientSocket status:500 message:@"Upload directory permissions could not be restricted; refusing upload" requestLogId:requestLogId];
        return;
    }
    ...
    int fd = open(destPath.fileSystemRepresentation, O_CREAT | O_EXCL | O_WRONLY, 0600);
    if (fd < 0) {
...
// HTTP quoted-string values cannot contain controls, quotes, or backslashes.
// The source filename remains unchanged; this is only the response header value.
static NSString *MCPHeaderSafeFilename(NSString *filename) {
    if (![filename isKindOfClass:[NSString class]] || filename.length == 0) {
        return @"file";
    }

    NSMutableString *sanitized = [NSMutableString stringWithCapacity:filename.length];
    BOOL hasSafeCharacter = NO;
    for (NSUInteger index = 0; index < filename.length; index++) {
        unichar character = [filename characterAtIndex:index];
        if (character < 0x20 || character == 0x7F || character == '"' || character == '\\') {
            [sanitized appendString:@"_"];
            continue;
        }
        [sanitized appendFormat:@"%C", character];
        hasSafeCharacter = YES;
    }
    return hasSafeCharacter ? sanitized : @"file";
}
...
- (void)handleDownloadFileRequestPath:(NSString *)path
    ...
    NSString *safeDownloadName = MCPHeaderSafeFilename(downloadName);
...
- (NSDictionary *)handleToolsList:(id)reqId {
    NSArray *tools = @[
        ...
        @{
            ...
            @"inputSchema": @{
                ...
                @"properties": @{
                    ...
                    @"include_semantic_state": @{@"type": @"boolean", @"description": @"Include bounded same-leaf AX semantic state with provenance (default: false)"},
                    @"include_semantic_diagnostics": @{@"type": @"boolean", @"description": @"Developer-only TEST-52 aggregate source probe; requires include_semantic_state=true and returns no values (default: false)"},
                    @"test52_read_only": @{@"type": @"boolean", @"description": @"TEST-52 only: one primary AX read, no bootstrap/VoiceOver fallback or retry; requires semantic state and diagnostics, forbids debug/value matching; returns semantic evidence only (default: false)"},
                    @"semantic_expected_value_sha256": @{@"type": @"string", @"description": @"Optional lowercase SHA-256 used for ephemeral nonsecure value comparison; never returned"},
        ...
        @{
            @"name": @"input_text",
            @"description": @"Dispatch text to the frontmost app via system keyboard events (fast, bulk input). dispatched=true only means the HID pipeline accepted the events - it does NOT mean the text reached the target field (delivery is unverified). Verify delivery via get_ui_elements with semantic_expected_value_sha256. If dispatch fails, retry once with type_text using the same text.",
            @"inputSchema": @{
                @"type": @"object",
                @"properties": @{
        ...
        @{
            @"name": @"type_text",
            @"description": @"Type text character-by-character through system keyboard text events, with HID fallback for ASCII keyboard characters. dispatched=true does NOT mean the text reached the target field (delivery is unverified). Use as the fallback when input_text fails.",
            @"inputSchema": @{
                @"type": @"object",
                @"properties": @{
        ...
        @{
            @"name": @"press_key",
            @"description": @"Press a special keyboard key. Dispatched only; delivery is not verified.",
            @"inputSchema": @{
                @"type": @"object",
                @"properties": @{
        ...
        @{
            @"name": @"list_dir",
            @"description": @"List the contents of a directory on the device filesystem. Returns each entry's name, type (file/directory/symlink/...), size, permission bits, and modification time. It runs directly in the MCP server process and does not invoke a shell or mcp-root fallback; inaccessible paths report their direct error. Combine with get_app_info to explore an app's data container.",
            @"inputSchema": @{
                @"type": @"object",
                @"properties": @{
        ...
        @{
            @"name": @"read_file",
            @"description": @"Read a file from the device filesystem. Text files return encoding 'utf8'; binary files (plist/db/images/dumps) return encoding 'base64'. Output is capped (default 512KB, max 4MB); when truncated, use GET /download_file?path=<path> to stream the full file. It runs directly in the MCP server process and does not invoke a shell or mcp-root fallback; inaccessible paths report their direct error.",
            @"inputSchema": @{
                @"type": @"object",
                @"properties": @{
        ...
        @{
            @"name": @"write_file",
            @"description": @"Write a file to the device filesystem, creating or overwriting it. content is interpreted per encoding: 'utf8' (default) or 'base64' for binary data. It runs directly in the MCP server process and does not invoke a shell or mcp-root fallback; inaccessible paths report their direct error. This is a mutating tool and is blocked while the device is locked or the screen is off.",
            @"inputSchema": @{
                @"type": @"object",
                @"properties": @{
...
- (NSDictionary *)handleToolsCall:(id)reqId params:(NSDictionary *)params {
    ...
    } else if ([toolName isEqualToString:@"create_visual_session"]) {
        return [self executeCreateVisualSession:reqId args:args];
    } else if ([toolName isEqualToString:@"analyze_visual_session"]) {
        return [self executeAnalyzeVisualSession:reqId args:args];
    ...
    } else if ([toolName isEqualToString:@"exp_keycode_tap"]) {
        return [self executeExpKeycode:reqId args:args];
    }
    // Enhanced gesture tools
...
- (NSDictionary *)fetchCompactElementsClickableOnly:(BOOL)clickableOnly {
    ...
    [[AccessibilityManager sharedInstance] getCompactUIElementsWithMaxElements:0
                                                          ...
                                                          includeSemanticState:NO
                                                   includeSemanticDiagnostics:NO
...
- (NSDictionary *)sanitizeUIElementsPayload:(NSDictionary *)payload
    ...
    MCPAddWhitelistedKeys(sanitized, payload, @[
        ...
        @"contextId",
        @"displayId",
        @"semantic_state_requested",
        @"semantic_state_elapsed_ms",
        @"device_generation",
        @"device_generation_source"
    ]);
    if (includeSemanticDiagnostics) {
        MCPAddWhitelistedKeys(sanitized, payload, @[
            @"semantic_diagnostics_requested",
            @"semantic_diagnostics",
            @"semantic_probe_limit",
            @"semantic_probe_skipped_count"
        ]);
    }
    ...
    if (elements) {
        ...
        for (id item in elements) {
            ...
            [sanitizedElements addObject:[self sanitizeUIElement:item debug:NO includeSemanticState:includeSemanticState]];
        }
...
- (NSDictionary *)executeGetUIElements:(id)reqId args:(NSDictionary *)args {
#if defined(PH_TEST52_NATIVE_CAPSULE_CLIENT) && defined(PH_TEST52_NATIVE_EVIDENCE_V3_ROUTE)
    if (args[@"test52_native_evidence_v3"] != nil) {
        NSSet *required=[NSSet setWithArray:@[@"test52_read_only",@"test52_native_c1",
            @"test52_native_evidence_v3",@"request_nonce",@"target_slot"]];
        if (![[NSSet setWithArray:args.allKeys] isEqual:required])
            return [self mcpError:reqId code:-32602 message:@"Native Evidence V3 requires an exact argument shape"];
        for (NSString *key in @[@"test52_read_only",@"test52_native_c1",@"test52_native_evidence_v3"]) {
            id value=args[key];
            if (![value isKindOfClass:NSNumber.class] ||
                CFGetTypeID((__bridge CFTypeRef)value)!=CFBooleanGetTypeID() || ![value boolValue])
                return [self mcpError:reqId code:-32602 message:@"Native Evidence V3 flags must be JSON true"];
        }
        if (![args[@"target_slot"] isEqual:@"hidden"] ||
            ![args[@"request_nonce"] isKindOfClass:NSString.class])
            return [self mcpError:reqId code:-32602 message:@"Native Evidence V3 target or nonce is invalid"];
        NSDictionary *capture=PHFENativeEvidenceV3CaptureHidden(args[@"request_nonce"]);
        BOOL passed=[capture[@"schema"] isEqual:@"ph.controlled_fixture.native_evidence.response.v3"] &&
            [capture[@"status"] isEqual:@"DIAGNOSTIC_ONLY"] &&
            [capture[@"hidden_state"] isEqual:@"TRUE"] &&
            [capture[@"hidden_presence"] isEqual:@"PRESENT_TRUE"] &&
            [capture[@"hidden_provenance"] isEqual:@"DIRECT_NATIVE_GETTER"] &&
            [capture[@"hidden_getter"] isEqual:@"TYPED_UIVIEW_ISHIDDEN"] &&
            [capture[@"controlled_presentation"] isEqual:@"NOT_PRESENTED_IN_CONTROLLED_FIXTURE"] &&
            [capture[@"authorization_capsule_issued"] isEqual:@NO] &&
            [capture[@"device_action_count"] isEqual:@0];
        return [self mcpSuccess:reqId structuredContent:capture isError:!passed];
    }
#else
    if (args[@"test52_native_evidence_v3"] != nil)
        return [self mcpError:reqId code:-32601 message:@"Native Evidence V3 route is not active"];
#endif
#if defined(PH_TEST52_NATIVE_CAPSULE_CLIENT) && defined(PH_TEST52_NATIVE_EVIDENCE_V2_ROUTE)
    if (args[@"test52_native_evidence_v2"] != nil) {
        NSSet *required=[NSSet setWithArray:@[@"test52_read_only",@"test52_native_c1",
            @"test52_native_evidence_v2",@"request_nonce",@"target_slot"]];
        if (![[NSSet setWithArray:args.allKeys] isEqual:required])
            return [self mcpError:reqId code:-32602 message:@"Native Evidence V2 requires an exact argument shape"];
        for (NSString *key in @[@"test52_read_only",@"test52_native_c1",@"test52_native_evidence_v2"]) {
            id value=args[key];
            if (![value isKindOfClass:NSNumber.class] ||
                CFGetTypeID((__bridge CFTypeRef)value)!=CFBooleanGetTypeID() || ![value boolValue])
                return [self mcpError:reqId code:-32602 message:@"Native Evidence V2 flags must be JSON true"];
        }
        if (![args[@"target_slot"] isEqual:@"disabled"] ||
            ![args[@"request_nonce"] isKindOfClass:NSString.class])
            return [self mcpError:reqId code:-32602 message:@"Native Evidence V2 target or nonce is invalid"];
        NSDictionary *capture=PHFENativeEvidenceV2CaptureDisabled(args[@"request_nonce"]);
        BOOL passed=[capture[@"schema"] isEqual:@"ph.controlled_fixture.native_evidence.response.v2"] &&
            [capture[@"status"] isEqual:@"DIAGNOSTIC_ONLY"] &&
            [capture[@"enabled_state"] isEqual:@"FALSE"] &&
            [capture[@"enabled_presence"] isEqual:@"PRESENT_FALSE"] &&
            [capture[@"enabled_provenance"] isEqual:@"DIRECT_NATIVE_GETTER"] &&
            [capture[@"enabled_getter"] isEqual:@"TYPED_UITEXTFIELD_ISENABLED"] &&
            [capture[@"authorization_capsule_issued"] isEqual:@NO] &&
            [capture[@"device_action_count"] isEqual:@0];
        return [self mcpSuccess:reqId structuredContent:capture isError:!passed];
    }
#else
    if (args[@"test52_native_evidence_v2"] != nil)
        return [self mcpError:reqId code:-32601 message:@"Native Evidence V2 route is not active"];
#endif
#if defined(PH_TEST52_NATIVE_CAPSULE_CLIENT) && defined(PH_TEST52_NATIVE_EVIDENCE_V1_ROUTE)
    if (args[@"test52_native_evidence_v1"] != nil) {
        NSSet *required=[NSSet setWithArray:@[@"test52_read_only",@"test52_native_c1",
            @"test52_native_evidence_v1",@"request_nonce",@"target_slot"]];
        if (![[NSSet setWithArray:args.allKeys] isEqual:required])
            return [self mcpError:reqId code:-32602 message:@"Native Evidence V1 requires an exact argument shape"];
        for (NSString *key in @[@"test52_read_only",@"test52_native_c1",@"test52_native_evidence_v1"]) {
            id value=args[key];
            if (![value isKindOfClass:NSNumber.class] ||
                CFGetTypeID((__bridge CFTypeRef)value)!=CFBooleanGetTypeID() || ![value boolValue])
                return [self mcpError:reqId code:-32602 message:@"Native Evidence V1 flags must be JSON true"];
        }
        if (![args[@"target_slot"] isEqual:@"read_only"] ||
            ![args[@"request_nonce"] isKindOfClass:NSString.class])
            return [self mcpError:reqId code:-32602 message:@"Native Evidence V1 target or nonce is invalid"];
        NSDictionary *capture=PHFENativeEvidenceV1CaptureReadOnly(args[@"request_nonce"]);
        BOOL passed=[capture[@"schema"] isEqual:@"ph.controlled_fixture.native_evidence.response.v1"] &&
            [capture[@"status"] isEqual:@"DIAGNOSTIC_ONLY"] &&
            [capture[@"editable_state"] isEqual:@"FALSE"] &&
            [capture[@"editable_presence"] isEqual:@"PRESENT_FALSE"] &&
            [capture[@"editable_provenance"] isEqual:@"DIRECT_NATIVE_GETTER"] &&
            [capture[@"editable_getter"] isEqual:@"TYPED_UITEXTVIEW_ISEDITABLE"] &&
            [capture[@"device_action_count"] isEqual:@0];
        return [self mcpSuccess:reqId structuredContent:capture isError:!passed];
    }
#else
    if (args[@"test52_native_evidence_v1"] != nil)
        return [self mcpError:reqId code:-32601 message:@"Native Evidence V1 route is not active"];
#endif
#ifdef PH_TEST52_FIELD_EVIDENCE
    if (args[@"test52_field_evidence"] != nil) {
        NSSet *required=[NSSet setWithArray:@[@"test52_read_only",@"test52_native_c1",
            @"test52_field_evidence",@"target_slot"]];
        if (![[NSSet setWithArray:args.allKeys] isEqual:required])
            return [self mcpError:reqId code:-32602 message:@"Field Evidence requires an exact read-only argument shape"];
        for (NSString *key in @[@"test52_read_only",@"test52_native_c1",@"test52_field_evidence"]) {
            id value=args[key];
            if (![value isKindOfClass:NSNumber.class] || CFGetTypeID((__bridge CFTypeRef)value)!=CFBooleanGetTypeID() || ![value boolValue])
                return [self mcpError:reqId code:-32602 message:@"Field Evidence flags must be JSON true"];
        }
        NSString *slot=[args[@"target_slot"] isKindOfClass:NSString.class] ? args[@"target_slot"] : nil;
        NSSet *slots=[NSSet setWithArray:@[@"A",@"B",@"secure",@"editable",@"read_only",@"disabled",@"hidden"]];
        if (![slots containsObject:slot]) return [self mcpError:reqId code:-32602 message:@"Unknown controlled Fixture target"];
        NSString *requestNonce=NSUUID.UUID.UUIDString.lowercaseString;
        NSDictionary *preEnvelope=PHFEReceiveCapture(slot,@"PRE",requestNonce);
        NSDictionary *pre=preEnvelope[@"snapshot"];
        if (![preEnvelope[@"status"] isEqual:@"DIAGNOSTIC_ONLY"] || !pre)
            return [self mcpSuccess:reqId structuredContent:preEnvelope isError:YES];

        __block NSDictionary *ax=nil; __block NSString *axError=nil; __block BOOL accepting=YES;
        NSObject *lock=[NSObject new]; dispatch_semaphore_t done=dispatch_semaphore_create(0);
        uint64_t axStarted=MCPFieldEvidenceNowNanoseconds();
        [[AccessibilityManager sharedInstance] getCompactUIElementsWithMaxElements:0
            visibleOnly:NO clickableOnly:NO includeSemanticState:YES includeSemanticDiagnostics:YES
            expectedValueSHA256:nil noBootstrapReadOnly:YES completion:^(NSDictionary *payload,NSString *error) {
                @synchronized(lock) { if (accepting) { ax=payload; axError=error; } }
                dispatch_semaphore_signal(done);
            }];
        BOOL timedOut=dispatch_semaphore_wait(done,dispatch_time(DISPATCH_TIME_NOW,10*NSEC_PER_SEC))!=0;
        uint64_t axFinished=MCPFieldEvidenceNowNanoseconds();
        @synchronized(lock) { accepting=NO; }
        if (timedOut || axError || !ax) return [self mcpSuccess:reqId structuredContent:@{
            @"status":@"UNAVAILABLE",@"authorization":@"BLOCKED",@"failure_code":timedOut ? @"AX_TIMEOUT" : @"AX_UNAVAILABLE",
            @"device_action_count":@0} isError:YES];

        NSDictionary *postEnvelope=PHFEReceiveCapture(slot,@"POST",requestNonce);
        NSDictionary *post=postEnvelope[@"snapshot"];
        if (![postEnvelope[@"status"] isEqual:@"DIAGNOSTIC_ONLY"] || !post)
            return [self mcpSuccess:reqId structuredContent:postEnvelope isError:YES];
        NSString *failure=nil;
        NSDictionary *result=PHFECorrelate(pre,post,ax,axStarted,axFinished,slot,requestNonce,
            PHFEExpectedRuntimeCDHash(),&failure);
        if (!result) return [self mcpSuccess:reqId structuredContent:@{
            @"status":@"UNAVAILABLE",@"authorization":@"BLOCKED",@"failure_code":failure ?: @"CORRELATION_FAILED",
            @"ax_identifier_probe":PHFEAXIdentifierProbeSummary(ax),
            @"device_action_count":@0} isError:YES];
        return [self mcpSuccess:reqId structuredContent:result isError:NO];
    }
#endif
#ifdef PH_TEST52_C1_DIAGNOSTIC
    if (args[@"test52_native_c1"] != nil) {
        NSDictionary *diagnostic = PHC1ReceiveDiagnostic(args);
        return [self mcpSuccess:reqId structuredContent:diagnostic
            isError:![diagnostic[@"status"] isEqual:@"DIAGNOSTIC_ONLY"]];
    }
#endif
    NSString *paramError = nil;
    id readOnlyValue = args[@"test52_read_only"];
    if (readOnlyValue && (![readOnlyValue isKindOfClass:[NSNumber class]] ||
        CFGetTypeID((__bridge CFTypeRef)readOnlyValue) != CFBooleanGetTypeID())) {
        return [self mcpError:reqId code:-32602 message:@"test52_read_only requires a JSON boolean"];
    }
    BOOL noBootstrapReadOnly = [readOnlyValue boolValue];
    double maxElementsValue = 0;
    double limitValue = 0;
    BOOL visibleOnly = YES;
    BOOL clickableOnly = NO;
    BOOL includeSemanticState = NO;
    BOOL includeSemanticDiagnostics = NO;
    BOOL debug = NO;
    NSString *expectedValueSHA256 = nil;
    if (!MCPNumberFromArgs(args, @"max_elements", 0, NO, &maxElementsValue, &paramError) ||
        ...
        !MCPBoolFromArgs(args, @"include_semantic_state", NO, &includeSemanticState, &paramError) ||
        !MCPBoolFromArgs(args, @"include_semantic_diagnostics", NO, &includeSemanticDiagnostics, &paramError) ||
        !MCPStringFromArgs(args, @"semantic_expected_value_sha256", NO, &expectedValueSHA256, &paramError) ||
        !MCPBoolFromArgs(args, @"debug", NO, &debug, &paramError)) {
        return [self mcpError:reqId code:-32602 message:paramError];
    }
    if (expectedValueSHA256.length > 0) {
        NSCharacterSet *nonHex = [[NSCharacterSet characterSetWithCharactersInString:@"0123456789abcdef"] invertedSet];
        if (!includeSemanticState || expectedValueSHA256.length != 64 ||
            [expectedValueSHA256 rangeOfCharacterFromSet:nonHex].location != NSNotFound) {
            return [self mcpError:reqId code:-32602 message:@"semantic_expected_value_sha256 requires include_semantic_state=true and exactly 64 lowercase hex characters"];
        }
    }
    if (includeSemanticDiagnostics && !includeSemanticState) {
        return [self mcpError:reqId code:-32602 message:@"include_semantic_diagnostics requires include_semantic_state=true"];
    }
    if (noBootstrapReadOnly) {
        NSSet *allowedKeys = [NSSet setWithArray:@[@"test52_read_only", @"include_semantic_state",
            @"include_semantic_diagnostics", @"visible_only", @"clickable_only", @"limit", @"max_elements", @"debug"]];
        if (!includeSemanticState || !includeSemanticDiagnostics || debug ||
            ![[NSSet setWithArray:args.allKeys] isSubsetOfSet:allowedKeys]) {
            return [self mcpError:reqId code:-32602 message:@"test52_read_only requires semantic state/diagnostics, debug=false, and observation-only parameters"];
        }
    }
    ...
    __block NSString *err = nil;
    __block BOOL acceptingResult = YES;
    ...
    [[AccessibilityManager sharedInstance] getCompactUIElementsWithMaxElements:compactMaxElements
                                                                ...
                                                                clickableOnly:clickableOnly
                                                         includeSemanticState:includeSemanticState
                                                   includeSemanticDiagnostics:includeSemanticDiagnostics
                                                    expectedValueSHA256:expectedValueSHA256
                                                          noBootstrapReadOnly:noBootstrapReadOnly
                                                                    completion:^(NSDictionary *result, NSString *error) {
    ...
    BOOL timedOut = dispatch_semaphore_wait(sem, dispatch_time(DISPATCH_TIME_NOW, 10 * NSEC_PER_SEC)) != 0;
    @synchronized (completionLock) {
        acceptingResult = NO;
    }

    if (noBootstrapReadOnly) {
        if (!payload || err || timedOut) {
            return [self mcpSuccess:reqId structuredContent:@{
                @"ok": @NO, @"observation_status": @"UNAVAILABLE", @"authorization_status": @"FAIL_CLOSED",
                @"observation_policy": @"test52_read_only_no_bootstrap", @"elements": @[],
                @"same_leaf_identity": @"unknown", @"same_leaf_identity_source": @"unavailable",
                @"device_generation_source": @"unavailable", @"freshness_source": @"unavailable",
                @"error": timedOut ? @"AX_READ_TIMEOUT" : @"AX_READ_UNAVAILABLE"
            } isError:YES];
        }
        NSMutableDictionary *responsePayload = [[self sanitizeUIElementsPayload:payload debug:NO
            includeSemanticState:YES includeSemanticDiagnostics:YES] mutableCopy];
        NSMutableArray *leaves = [NSMutableArray array];
        for (NSDictionary *element in responsePayload[@"elements"]) {
            NSMutableDictionary *leaf = [NSMutableDictionary dictionary];
            MCPAddWhitelistedKeys(leaf, element, @[@"semantic_leaf_ref", @"semantic_state"]);
            [leaves addObject:leaf];
        }
        responsePayload[@"elements"] = leaves;
        responsePayload[@"observation_status"] = @"AVAILABLE";
        responsePayload[@"authorization_status"] = @"FAIL_CLOSED";
        responsePayload[@"observation_policy"] = @"test52_read_only_no_bootstrap";
        return [self mcpSuccess:reqId structuredContent:responsePayload];
    }

    if (payload) {
        NSDictionary *responsePayload = [self sanitizeUIElementsPayload:payload
                                                                   debug:debug
                                                    includeSemanticState:includeSemanticState
                                              includeSemanticDiagnostics:includeSemanticDiagnostics];
        return [self mcpSuccess:reqId structuredContent:responsePayload];
...
- (void)purgeExpiredVisualSessionsAtTime:(NSTimeInterval)now {
    for (NSString *sessionId in [_temporaryVisualSessions.allKeys copy]) {
        NSDictionary *record = _temporaryVisualSessions[sessionId];
        NSNumber *expiration = [record[@"expiration_time"] isKindOfClass:[NSNumber class]] ? record[@"expiration_time"] : nil;
        if (!expiration || expiration.doubleValue <= now) {
            [_temporaryVisualSessions removeObjectForKey:sessionId];
        }
    }
}
...
- (NSDictionary *)consumeVisualSessionWithID:(NSString *)sessionId atTime:(NSTimeInterval)now {
    @synchronized (self) {
        [self purgeExpiredVisualSessionsAtTime:now];
        NSDictionary *record = _temporaryVisualSessions[sessionId];
        if (!record || ![record[@"status"] isEqualToString:@"ACTIVE"] ||
            ![record[@"permission_scope"] isEqualToString:MCP_VISUAL_SESSION_SCOPE] ||
            ![record[@"one_time_access"] boolValue]) {
            return nil;
        }

        // Remove while holding the registry lock. A duplicate request cannot
        // reach OCR after this point, regardless of OCR success or failure.
        [_temporaryVisualSessions removeObjectForKey:sessionId];
        return record;
    }
}
...
- (NSDictionary *)executeCreateVisualSession:(id)reqId args:(NSDictionary *)args {
    // This internal grant endpoint is intentionally omitted from tools/list.
    // It accepts no caller-controlled metadata, scope, lifetime, or content.
    if (args.count != 0) {
        return [self mcpError:reqId code:-32602 message:@"Invalid local visual session arguments"];
    }

    NSString *token = MCPSecureRandomHexString();
    NSString *ownerToken = MCPSecureRandomHexString();
    if (token.length != 32 || ownerToken.length != 32) {
        return [self mcpSuccess:reqId text:@"Local visual session unavailable" isError:YES];
    }

    NSTimeInterval now = [[NSDate date] timeIntervalSince1970];
    long long createdSeconds = (long long)now;
    NSNumber *createdTime = @(createdSeconds);
    NSNumber *expirationTime = @(createdSeconds + MCP_VISUAL_SESSION_TTL_SECONDS);
    NSString *sessionId = [@"visualsession." stringByAppendingString:token];
    NSDictionary *record = @{
        @"session_id": sessionId,
        @"owner_id": [@"visualowner." stringByAppendingString:ownerToken],
        @"created_time": createdTime,
        @"expiration_time": expirationTime,
        @"permission_scope": MCP_VISUAL_SESSION_SCOPE,
        @"one_time_access": @YES,
        @"status": @"ACTIVE",
    };
    ...
    @synchronized (self) {
        [self purgeExpiredVisualSessionsAtTime:now];
        if (_temporaryVisualSessions.count >= MCP_VISUAL_SESSION_MAX_ACTIVE) {
            return [self mcpSuccess:reqId text:@"Local visual session capacity reached" isError:YES];
        }
        _temporaryVisualSessions[sessionId] = record;
    }

    // owner_id remains device-private. The host receives only the short-lived
    // authorization metadata it needs to bind its in-memory session.
    return [self mcpSuccess:reqId structuredContent:@{
        @"session_id": sessionId,
        @"created_time": createdTime,
        @"expiration_time": expirationTime,
...
- (NSDictionary *)executeAnalyzeVisualSession:(id)reqId args:(NSDictionary *)args {
    // This internal read-only bridge is intentionally omitted from tools/list.
    // It accepts no text, geometry, action, scope, or lifetime arguments.
    NSSet<NSString *> *allowedKeys = [NSSet setWithArray:@[@"session_id", @"recognition_mode"]];
    for (id key in args) {
    ...
    NSString *recognitionMode = nil;
    NSString *paramError = nil;
    if (!MCPStringFromArgs(args, @"session_id", YES, &sessionId, &paramError) ||
        !MCPStringFromArgs(args, @"recognition_mode", NO, &recognitionMode, &paramError)) {
        return [self mcpError:reqId code:-32602 message:@"Invalid local visual session arguments"];
    }
    if (!MCPIsWellFormedVisualSessionID(sessionId)) {
        return [self mcpError:reqId code:-32602 message:@"Invalid or expired local visual session"];
    }
    if (recognitionMode.length == 0) recognitionMode = @"accurate";
    if (!([recognitionMode isEqualToString:@"accurate"] || [recognitionMode isEqualToString:@"fast"])) {
        return [self mcpError:reqId code:-32602 message:@"Invalid local visual recognition mode"];
    }

    if (![self consumeVisualSessionWithID:sessionId atTime:[[NSDate date] timeIntervalSince1970]]) {
        return [self mcpError:reqId code:-32602 message:@"Invalid or expired local visual session"];
    }
    ...
    @try {
        result = [[OCRManager sharedInstance] recognizeTemporaryVisualSessionWithRecognitionMode:recognitionMode error:&error];
    } @catch (NSException *exception) {
        // Vision is a private-session input only. Never allow an unexpected
        // framework exception to abort the MCP connection or expose frame data.
        MCP_LOG(@"local visual recognition exception class=%@", exception.name ?: @"unknown");
        error = @"Local visual recognition failed";
    }
    if (!result) {
        // Do not propagate capture/OCR details or user-visible text through MCP.
        return [self mcpSuccess:reqId text:@"Local visual recognition failed" isError:YES];
    }
    return [self mcpSuccess:reqId structuredContent:result];
}
...
- (NSDictionary *)executeInputText:(id)reqId args:(NSDictionary *)args {
    ...
    // Contract honesty: "dispatched" only means the HID events were accepted
    // by the pipeline addressed to the frontmost app. It does NOT mean the
    // text reached the target field; delivery verification is the host's job
    // (get_ui_elements + semantic_expected_value_sha256).
    NSString *method = nil;
    if (ok && [err isEqualToString:@"Used HID fallback for ASCII text"]) {
        method = @"hid_fallback";
    } else if (ok) {
        method = @"unicode_bulk";
    }
    ...
    if (err.length > 0 && ![err isEqualToString:@"Used HID fallback for ASCII text"]) {
        structured[@"reason"] = err;
    }

    if (ok) {
        return [self mcpSuccess:reqId
                           text:[NSString stringWithFormat:@"Input dispatched (delivery unverified): %lu characters", (unsigned long)text.length]
              structuredContent:structured
                       isError:NO];
    }
    return [self mcpSuccess:reqId
                       text:[NSString stringWithFormat:@"Input failed: %@", err ?: @"timeout"]
          structuredContent:structured
                   isError:YES];
}
...
- (NSDictionary *)executeTypeText:(id)reqId args:(NSDictionary *)args {
    ...
    NSMutableDictionary *structured = [NSMutableDictionary dictionaryWithDictionary:@{
        ...
        @"dispatched": @(ok),
        @"delivery": @"unverified",
        @"method": @"hid_keycode_per_char"
    }];

    if (ok) {
        NSString *msg = [NSString stringWithFormat:@"Typed dispatched (delivery unverified): %lu characters", (unsigned long)text.length];
        if (err) msg = [msg stringByAppendingFormat:@" (%@)", err];
        return [self mcpSuccess:reqId text:msg structuredContent:structured isError:NO];
    }
    structured[@"reason"] = err ?: @"timeout";
    return [self mcpSuccess:reqId text:[NSString stringWithFormat:@"Type failed: %@", err ?: @"timeout"] structuredContent:structured isError:YES];
}
...
- (NSDictionary *)executePressKey:(id)reqId args:(NSDictionary *)args {
    ...
    if (ok) {
        return [self mcpSuccess:reqId
                           text:[NSString stringWithFormat:@"Key dispatched (delivery unverified): %@", key]
              structuredContent:[NSMutableDictionary dictionaryWithDictionary:@{
                  @"accepted": @YES,
                  @"dispatched": @YES,
                  @"delivery": @"unverified"
              }]
...
// Batch 1D E1-B experiment entry (hidden; not in tools/list): single keycode
// tap through the proven broadcast path (same sender value as touch events).
// Experiment build only — the production input contract is input_text/type_text.
- (NSDictionary *)executeExpKeycode:(id)reqId args:(NSDictionary *)args {
    double page = 7;
    double usage = 0;
    NSString *paramError = nil;
    if (!MCPNumberFromArgs(args, @"usage", 0, YES, &usage, &paramError)) {
        return [self mcpError:reqId code:-32602 message:paramError];
    }
    if (!MCPNumberFromArgs(args, @"page", 7, NO, &page, &paramError)) {
        return [self mcpError:reqId code:-32602 message:paramError];
    }
    ...
    __block NSString *err = nil;
    dispatch_semaphore_t sem = dispatch_semaphore_create(0);
    [[IOSMCPHIDManager sharedInstance] expKeyboardTapWithPage:(uint32_t)page usage:(uint32_t)usage completion:^(BOOL success, NSString *error) {
        ok = success;
        err = error;
        dispatch_semaphore_signal(sem);
    }];
    ...
    NSMutableDictionary *structured = [NSMutableDictionary dictionaryWithDictionary:@{
        ...
        @"dispatched": @(ok),
        @"delivery": @"unverified",
        @"method": @"keycode_broadcast"
    }];
    if (ok) {
        return [self mcpSuccess:reqId text:@"Keycode dispatched (delivery unverified)" structuredContent:structured isError:NO];
    }
    return [self mcpSuccess:reqId text:[NSString stringWithFormat:@"Keycode dispatch failed: %@", err ?: @"timeout"] structuredContent:structured isError:YES];
}
