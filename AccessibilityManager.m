...
- (void)getCompactUIElementsWithMaxElements:(NSInteger)maxElements
    ...
    [self getCompactUIElementsWithMaxElements:maxElements
                                 visibleOnly:visibleOnly
                               clickableOnly:clickableOnly
                        includeSemanticState:includeSemanticState
                  includeSemanticDiagnostics:includeSemanticDiagnostics
                         expectedValueSHA256:expectedValueSHA256
                         noBootstrapReadOnly:NO
...
- (void)getCompactUIElementsWithMaxElements:(NSInteger)maxElements
                 ...
                 includeSemanticDiagnostics:(BOOL)includeSemanticDiagnostics
                        ...
                        noBootstrapReadOnly:(BOOL)noBootstrapReadOnly
                                 completion:(void (^)(NSDictionary *, NSString *))completion {
    if (noBootstrapReadOnly && (!includeSemanticState || !includeSemanticDiagnostics || expectedValueSHA256 != nil)) {
        if (completion) completion(nil, @"AX_READ_ONLY_OPTIONS_INVALID");
        return;
    }
    ...
    dispatch_async(_axQueue, ^{
        @try {
            ...
            if (!context || context.pid <= 0) {
                if (completion) completion(nil, noBootstrapReadOnly ? @"AX_CONTEXT_UNAVAILABLE" : @"No frontmost application context available");
                return;
            }
            ...
            NSDictionary *payload = [self compactPayloadForContext:context
                                         ...
                                         expectedValueSHA256:expectedValueSHA256
                                                             error:&compactError];
            // Request-local diagnostic policy. Exit before bootstrap policy,
            // retries, context refresh, or any alternate observation path.
            if (noBootstrapReadOnly) {
                NSArray *elements = [payload[@"elements"] isKindOfClass:[NSArray class]] ? payload[@"elements"] : nil;
                BOOL available = elements.count > 0 && compactError.length == 0;
                for (id element in elements) {
                    if (![element isKindOfClass:[NSDictionary class]]) available = NO;
                }
                if (completion) completion(available ? payload : nil, available ? nil : @"AX_READ_UNAVAILABLE");
                return;
            }
            if (payload) {
            ...
            if (noBootstrapReadOnly) {
                if (completion) completion(nil, @"AX_READ_UNAVAILABLE");
                return;
            }
