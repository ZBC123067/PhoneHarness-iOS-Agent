...
#define IOS_MCP_AUTH_TOKEN_PREFERENCE_KEY @"authToken"
#define IOS_MCP_ALLOW_LAN_PREFERENCE_KEY @"allowLan"
...
static inline NSString *IOSMCPConfiguredAuthTokenValue(void) {
    CFPreferencesAppSynchronize((__bridge CFStringRef)IOS_MCP_PREFERENCES_DOMAIN);
    CFPropertyListRef value = CFPreferencesCopyAppValue((__bridge CFStringRef)IOS_MCP_AUTH_TOKEN_PREFERENCE_KEY,
                                                        (__bridge CFStringRef)IOS_MCP_PREFERENCES_DOMAIN);
    NSString *token = nil;
    if (value) {
        if (CFGetTypeID(value) == CFStringGetTypeID()) {
            token = [(__bridge NSString *)value copy];
        }
        CFRelease(value);
    }
    return token;
...
static inline BOOL IOSMCPAllowLanAccess(void) {
    CFPreferencesAppSynchronize((__bridge CFStringRef)IOS_MCP_PREFERENCES_DOMAIN);
    CFPropertyListRef value = CFPreferencesCopyAppValue((__bridge CFStringRef)IOS_MCP_ALLOW_LAN_PREFERENCE_KEY,
                                                        (__bridge CFStringRef)IOS_MCP_PREFERENCES_DOMAIN);
    BOOL allowed = NO;
    if (value) {
        if (CFGetTypeID(value) == CFBooleanGetTypeID()) {
            ...
            allowed = [text isEqualToString:@"yes"] ||
