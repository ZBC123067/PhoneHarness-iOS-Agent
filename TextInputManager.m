...
- (void)inputText:(NSString *)text completion:(void (^)(BOOL, NSString *))completion {
    ...
    dispatch_async(_inputQueue, ^{
        // Contract honesty: with no frontmost text target there is nowhere to
        // deliver text, so dispatch must not be claimed (nor may the ASCII
        // fallback fire system-wide events). "no_target" maps to
        // dispatched:false + reason:"no_target" in the tool response.
        pid_t targetPid = frontmostTextTargetPid();
        if (targetPid <= 0) {
            if (completion) completion(NO, @"no_target");
            return;
        }
