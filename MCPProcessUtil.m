...
static BOOL MCPInitSpawnAttributes(posix_spawnattr_t *attr) {
    if (posix_spawnattr_init(attr) != 0) return NO;
    ...
    // The timeout path must terminate the shell and all descendants that
    // inherit its stdout/stderr pipe. Fail closed if isolation is unavailable.
...
BOOL MCPRunProcess(NSString *launchPath,
    ...
    if (!MCPInitSpawnAttributes(&attr)) {
        MCPFreeCStringArray(argv, argvStrings.count);
        if (environmentOverrides.count > 0) {
        ...
        close(pipeFDs[0]);
        close(pipeFDs[1]);
        if (errorMessage) *errorMessage = @"Failed to configure isolated process group";
        return NO;
    }
